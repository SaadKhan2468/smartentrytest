...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <>
      ...
      <div
        className={`bg-white shadow-md lg:h-screen lg:sticky top-0 overflow-y-auto transition-transform hide-scrollbar lg:block hidden transform border border-r-[#E3E3E3] ${isOpen ? "translate-x-0" : "-translate-x-full"
          } md:translate-x-0 sidebar ${isOpen
            ? "w-28 sidebar-collapsed"
            : "2xl:w-96 lg:w-80 sidebar-expanded"
          }`}
      >
        <div
          className={`flex flex-col   pt-9  ${isOpen ? "space-y-6" : ""
            }`}
        >
          <Link to="/home">
            <h1
              className={`uppercase font-bold px-5 text-[#1976d2]  ${isOpen ? "text-lg" : " text-[1.2rem]"
                }`}
            >
              PROCTOR AI{" "}
            </h1>
          ...
          <nav className="w-full mt-16">
            <ul
              className={`2xl:space-y-6 space-y-4 ${isOpen ? "flex flex-col items-center" : ""
                }`}
            >
              <li
                className={isActive([
              ...
              >
                <Link
                  to="/"
                  className={`flex ${isOpen ? "justify-center p-3" : "py-4 pl-5"
              ...
              <h1
                className={`text-base font-normal text-black poppins ml-5 text-start ${isOpen ? "hidden" : "block"
                  }`}
              >
                Account Page{" "}
              </h1>
      ...
      <div
        className={`bg-white shadow-md lg:hidden h-screen fixed top-0 overflow-y-auto hide-scrollbar  w-72 left-0  transition-transform duration-300 z-50 sidebar-mobile ${isOpen ? "translate-x-0" : "-translate-x-full"
          }`}
      >
