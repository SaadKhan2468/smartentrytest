...
const ViewInstructionPage = () => {
  ...
  const fromPath = location.state?.from || '/home'; // Default to home if no state passed
    ...
    <>
      <section className="px-[3%] py-7">
        ...
        <div className="md:my-7 my-3 sm:border border border-border-clr lg:space-y-10 space-y-5 rounded-lg md:p-5 p-3 ">
          ...
          <button onClick={() => navigate('/')} className="font-semibold text-black md:text-xl text-sm">
            Back
          </button>
          <button onClick={() => navigate('/practicetest', { state: { from: fromPath } })} className="font-semibold text-black md:text-xl text-sm">
            Next
          </button>
          </div>
        </div>
